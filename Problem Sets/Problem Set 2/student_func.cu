                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  //__shared__ float *sh_inputChannel=new float[numRows*numCols];
  //sh_inputChannel[thread_1D_pos]=inputChannel[thread_1D_pos];
  ...
  for(int r=-filterWidth/2;r<=filterWidth/2;++r)
	  ...
	  {
		  int x=min(max(thread_2D_pos.y+r, 0), static_cast<int>(numRows-1));
		  int y=min(max(thread_2D_pos.x+c, 0), static_cast<int>(numCols-1));
		  
		  float filter_value=filter[(r+filterWidth/2)*filterWidth + (c+filterWidth/2)];
		  
		  int pos=x * numCols + y;
		  
		  result+= inputChannel[pos] * filter_value;
	  }	    
